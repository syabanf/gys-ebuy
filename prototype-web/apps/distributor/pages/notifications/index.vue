...
import { GBadge, GButton, GCard, formatDateTime } from '@gys/ui'
import { useNotifications } from '~/composables/useNotifications'

const router = useRouter()
const { items, unread, markRead, markAllRead } = useNotifications()

const iconFor: Record<string, { icon: string; tone: 'brand' | 'info' | 'warning' | 'success' | 'neutral' }> = {
  order: { icon: '◫', tone: 'brand' },
  stock: { icon: '▼', tone: 'warning' },
  payment: { icon: '◉', tone: 'success' },
    ...
    <GCard :padded="false">
      <ul class="divide-y divide-neutral-100">
        <li
          ...
          @click="open(n)"
        >
          <span class="h-9 w-9 rounded-full flex items-center justify-center text-sm flex-shrink-0"
            :class="{ 'bg-brand-50 text-brand-700': iconFor[n.type].tone === 'brand', 'bg-amber-50 text-amber-700': iconFor[n.type].tone === 'warning', 'bg-emerald-50 text-emerald-700': iconFor[n.type].tone === 'success', 'bg-neutral-100 text-neutral-600': iconFor[n.type].tone === 'neutral' }">
            {{ iconFor[n.type].icon }}
          </span>
